...
Now you can rename `src/Index.css` to `src/Index.scss` and update `src/Index.jsst.js` to import `src/Index.scss`.
...
Import required reactstrap components within `src/Index.jsst.js` file or your custom component files:

```js
import { Button } from 'reactstrap';
```

Now you are ready to use the imported reactstrap components within your component hierarchy defined in the render method. Here is an example [`TodoListoList.js`](https://gist.githubusercontent.com/zx6658/d9f128cd57ca69e583ea2b5fea074238/raw/a56701c142d0c622eb6c20a457fbc01d708cb485/Index.js) redone using reactstrap.
...
4. Add `// @flow` to any files you want to type check (for example, to `src/Index.jsst.js`).
...
To try it, delete all the code in `src/Index.jsst.js` and replace it with any of the examples on its website. The [Basic Example](https://reacttraining.com/react-router/web/example/basic) is a good place to get started.
...
We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `Index.test.js` and `Index.jsst.js` are in the same folder, the test just needs to `import Index from './Index'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
| INLINE_RUNTIME_CHUNK |          :x:           | :white_check_mark: | By default, Create React Index will embed the runtime script into `index.html` during the production build. When set to `false`, the script will not be embedded and will be imported as usual. This is normally required when dealing with CSP.                                                                                                                                                                                                                                                                                                                                                                                                                       |
